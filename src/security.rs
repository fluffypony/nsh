...
fn has_obfuscation(cmd: &str) -> Option<(RiskLevel, &'static str)> {
    ...
    if cmd.contains("$(") {
        ...
        if has_unsafe {
            ...
            if has_eval {
                return Some((
                    RiskLevel::Elevated,
                    "dynamic evaluation with command substitution",
                ));
            }
    ...
    if cmd.contains('`') {
        let lower = cmd.to_lowercase();
        let has_risky = lower.contains("`curl")
            || lower.contains("`wget")
...
fn assess_single_command(argv: &[&str]) -> (RiskLevel, Option<&'static str>) {
    ...
    match program.as_str() {
        ...
        "git" => {
            let has_force_delete = rest.contains(&"-D");
...
mod tests {
    ...
    fn test_backtick_with_curl_is_elevated() {
        ...
        assert_eq!(
            reason,
            Some("backtick substitution with network/eval command")
        );
    }
    ...
    fn test_command_substitution_with_curl_pipe_shell_is_dangerous() {
        ...
        assert_eq!(
            reason,
            Some("remote code execution via command substitution")
        );
    }
