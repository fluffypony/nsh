...
pub fn main_inner() -> anyhow::Result<()> {
    ...
    // apply_pending_update already writes update_notice when a new binary is installed.
    // Shell hooks detect hook drift via periodic hash checks (every 20 cmds / 60s cooldown),
    // so no subprocess spawn is needed here.
